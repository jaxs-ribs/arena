...
pub fn handle_segmented_reduce_sum(binds: &[BufferView]) -> Result<Vec<Vec<u8>>, ComputeError> {
    ...
    for i in 0..segment_indices.len() {
        ...
        let segment_end = if i + 1 < segment_indices.len() { segment_indices[i+1] as usize } else { data_values.len() };
...
mod tests {
    ...
    fn mock_segmented_reduce_sum_computes_segment_sums() {
        ...
        let segment_indices_bytes: StdArc<[u8]> = bytemuck::cast_slice(&segment_indices).to_vec().into();
        let segment_indices_buffer_view = BufferView::new(segment_indices_bytes, vec![segment_indices.len()], std::mem::size_of::<u32>());

        let output_placeholder: StdArc<[u8]> = vec![0u8; expected_sums.len() * std::mem::size_of::<f32>()].into();
        let output_buffer_view = BufferView::new(output_placeholder, vec![expected_sums.len()], std::mem::size_of::<f32>());
        ...
        let dispatch_binds = [input_buffer_view, segment_indices_buffer_view, output_buffer_view, config_buffer_view];
        let result_buffers = cpu.dispatch(&Kernel::SegmentedReduceSum, &dispatch_binds, [1,1,1]).expect("Dispatch for SegmentedReduceSum failed");

        assert_eq!(result_buffers.len(), 1);
        let output_bytes = &result_buffers[0];
        assert_eq!(output_bytes.len(), expected_sums.len() * std::mem::size_of::<f32>());
        ...
        for (i, (got, expected)) in output_values.iter().zip(expected_sums.iter()).enumerate() {
            assert!((got - expected).abs() < 1e-6, "Mismatch for SegmentedReduceSum at segment index {}. Got: {}, Expected: {}", i, got, expected);
        }
    }
}
