...
mod tests {
    ...
    fn mock_integrate_bodies_updates_sphere() {
        ...
        let initial_sphere = TestSphere { pos: TestVec3 { x: 0.0, y: 10.0, z: 0.0 }, vel: TestVec3 { x: 1.0, y: 0.0, z: 0.0 } };
        let spheres_data = vec![initial_sphere];
        let sphere_bytes: StdArc<[u8]> = bytemuck::cast_slice(&spheres_data).to_vec().into();
        let sphere_buffer_view = BufferView::new(sphere_bytes, vec![spheres_data.len()], std::mem::size_of::<TestSphere>());

        let params = TestPhysParams { gravity: TestVec3 { x: 0.0, y: -9.81, z: 0.0 }, dt: 0.1, _padding1: 0.0, _padding2: 0.0 };
        let params_bytes: StdArc<[u8]> = bytemuck::bytes_of(&params).to_vec().into();
        let params_buffer_view = BufferView::new(params_bytes, vec![1], std::mem::size_of::<TestPhysParams>());

        let result_buffers = cpu.dispatch(&Kernel::IntegrateBodies, &[sphere_buffer_view.clone(), params_buffer_view], [1,1,1]).expect("Dispatch for IntegrateBodies failed");
