...
mod tests {
    ...
    fn mock_expand_instances_repeats_template() {
        ...
        let template_buffer_view = BufferView::new(template_bytes, vec![template_data.len()], std::mem::size_of::<f32>());

        let output_placeholder_total_bytes = template_data.len() * repetition_count as usize * std::mem::size_of::<f32>();
        let output_placeholder: StdArc<[u8]> = vec![0u8; output_placeholder_total_bytes].into();
        let output_buffer_view = BufferView::new(output_placeholder, vec![repetition_count as usize, template_data.len()], std::mem::size_of::<f32>());
        ...
        let config_buffer_view = BufferView::new(config_bytes, vec![1], std::mem::size_of::<TestExpandConfig>());

        let dispatch_binds = [template_buffer_view, output_buffer_view, config_buffer_view];
        let result_buffers = cpu.dispatch(&Kernel::ExpandInstances, &dispatch_binds, [1,1,1]).expect("Dispatch for ExpandInstances failed");
