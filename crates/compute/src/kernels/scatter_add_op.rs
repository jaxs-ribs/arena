...
mod tests {
    ...
    fn mock_scatter_add_adds_values_to_indices() {
        ...
        let values_to_add_bytes: StdArc<[u8]> = bytemuck::cast_slice(&values_to_add_data).to_vec().into();
        let values_buffer_view = BufferView::new(values_to_add_bytes, vec![values_to_add_data.len()], std::mem::size_of::<f32>());

        let indices_bytes: StdArc<[u8]> = bytemuck::cast_slice(&indices_data).to_vec().into();
        let indices_buffer_view = BufferView::new(indices_bytes, vec![indices_data.len()], std::mem::size_of::<u32>());

        let initial_output_bytes: StdArc<[u8]> = bytemuck::cast_slice(&initial_output_data).to_vec().into();
        let output_accumulator_buffer_view = BufferView::new(initial_output_bytes, vec![initial_output_data.len()], std::mem::size_of::<f32>());
        ...
        let dispatch_binds = [values_buffer_view, indices_buffer_view, output_accumulator_buffer_view, config_buffer_view];
        let result_buffers = cpu.dispatch(&Kernel::ScatterAdd, &dispatch_binds, [1,1,1]).expect("Dispatch for ScatterAdd failed");

        assert_eq!(result_buffers.len(), 1);
        let output_bytes = &result_buffers[0];
        assert_eq!(output_bytes.len(), expected_final_output_data.len() * std::mem::size_of::<f32>());
        ...
        for (i, (got, expected)) in output_values.iter().zip(expected_final_output_data.iter()).enumerate() {
            assert!((got - expected).abs() < 1e-6, "Mismatch for ScatterAdd at index {}. Got: {}, Expected: {}", i, got, expected);
        }
    }
}
