...
mod tests {
    ...
    fn mock_gather_collects_values_from_indices() {
        ...
        let source_buffer_view = BufferView::new(source_data_bytes, vec![source_data.len()], std::mem::size_of::<f32>());

        let indices_bytes: StdArc<[u8]> = bytemuck::cast_slice(&indices_to_gather).to_vec().into();
        let indices_buffer_view = BufferView::new(indices_bytes, vec![indices_to_gather.len()], std::mem::size_of::<u32>());

        let output_placeholder: StdArc<[u8]> = vec![0u8; expected_output_data.len() * std::mem::size_of::<f32>()].into();
        let output_buffer_view = BufferView::new(output_placeholder, vec![expected_output_data.len()], std::mem::size_of::<f32>());
        ...
        let dispatch_binds = [source_buffer_view, indices_buffer_view, output_buffer_view, config_buffer_view];
        let result_buffers = cpu.dispatch(&Kernel::Gather, &dispatch_binds, [1,1,1]).expect("Dispatch for Gather failed");
        ...
        for (i, (got, expected)) in output_values.iter().zip(expected_output_data.iter()).enumerate() {
            assert!((got - expected).abs() < 1e-6, "Mismatch for Gather at index {}. Got: {}, Expected: {}", i, got, expected);
        }
    }
}
