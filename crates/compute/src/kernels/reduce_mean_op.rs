...
pub fn handle_reduce_mean(binds: &[BufferView]) -> Result<Vec<Vec<u8>>, ComputeError> {
    ...
    let mean_value: f32 = if count == 0 { 0.0 } else { input_values.iter().sum::<f32>() / count as f32 };
...
mod tests {
    ...
    fn mock_reduce_mean_computes_mean() {
        ...
        let result_buffers1 = cpu.dispatch(&Kernel::ReduceMean, &dispatch_binds1, [1,1,1]).expect("Dispatch for ReduceMean (case 1) failed");
        ...
        let result_buffers2 = cpu.dispatch(&Kernel::ReduceMean, &dispatch_binds2, [1,1,1]).expect("Dispatch for ReduceMean (case 2) failed");
