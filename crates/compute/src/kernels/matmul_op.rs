...
pub fn handle_matmul(binds: &[BufferView]) -> Result<Vec<Vec<u8>>, ComputeError> {
    ...
    if a_view.element_size_in_bytes != std::mem::size_of::<f32>() || b_view.element_size_in_bytes != std::mem::size_of::<f32>() {
...
mod tests {
    ...
    fn mock_matmul_multiplies_matrices() {
        ...
        let a_buffer_view = BufferView::new(a_bytes, vec![m as usize, k as usize], std::mem::size_of::<f32>());

        let b_bytes: StdArc<[u8]> = bytemuck::cast_slice(&b_data).to_vec().into();
        let b_buffer_view = BufferView::new(b_bytes, vec![k as usize, n as usize], std::mem::size_of::<f32>());

        let output_placeholder: StdArc<[u8]> = vec![0u8; (m * n) as usize * std::mem::size_of::<f32>()].into();
        let output_buffer_view = BufferView::new(output_placeholder, vec![m as usize, n as usize], std::mem::size_of::<f32>());

        let matmul_config = TestMatMulConfig { m, k, n };
        let config_bytes: StdArc<[u8]> = bytemuck::bytes_of(&matmul_config).to_vec().into();
        let config_buffer_view = BufferView::new(config_bytes, vec![1], std::mem::size_of::<TestMatMulConfig>());

        let dispatch_binds = [a_buffer_view, b_buffer_view, output_buffer_view, config_buffer_view];
        let result_buffers = cpu.dispatch(&Kernel::MatMul, &dispatch_binds, [1,1,1]).expect("Dispatch for MatMul failed");
        ...
        for (i, (got, expected)) in output_values.iter().zip(expected_output_data.iter()).enumerate() {
            assert!((got - expected).abs() < 1e-6, "Mismatch for MatMul at index {}. Got: {}, Expected: {}", i, got, expected);
        }
    }
}
