...
pub fn handle_min(binds: &[BufferView]) -> Result<Vec<Vec<u8>>, ComputeError> {
    if binds.len() < 4 { // IN1, IN2, OUT, CONFIG per layout.rs
        return Err(ComputeError::ShapeMismatch("Min kernel expects 4 buffers (input_a, input_b, output_placeholder, config)"));
    }
...
mod tests {
    ...
    fn mock_min_returns_minimum_values() {
        ...
        let expected_output_data: Vec<f32> = input_a_data.iter()
