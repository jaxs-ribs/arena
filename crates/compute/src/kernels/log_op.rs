...
pub fn handle_log(binds: &[BufferView]) -> Result<Vec<Vec<u8>>, ComputeError> {
    if binds.len() < 3 { // IN, OUT_placeholder, CONFIG per layout.rs
        return Err(ComputeError::ShapeMismatch("Log kernel expects 3 buffers (input, output_placeholder, config)"));
    }
...
mod tests {
    ...
    fn mock_log_computes_logarithm() {
        ...
        let result_buffers = cpu.dispatch(&Kernel::Log, &[input_buffer_view, output_buffer_view, config_buffer_view], workgroups)
        ...
        for (got, expected) in output_values.iter().zip(expected_output_data.iter()) {
            if expected.is_finite() {
                ...
                assert!(got.is_nan() && expected.is_nan() || got.is_infinite() && expected.is_infinite(),
                        "Mismatch for Log non-finite. Got: {}, Expected: {}", got, expected);
            }
        }
    }
} 