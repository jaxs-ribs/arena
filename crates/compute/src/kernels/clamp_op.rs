...
pub fn handle_clamp(binds: &[BufferView]) -> Result<Vec<Vec<u8>>, ComputeError> {
    if binds.len() < 5 { // value, min_val, max_val, out_placeholder, config per layout.rs
    ...
    if value_view.element_size_in_bytes != std::mem::size_of::<f32>() || 
       ...
       max_view.element_size_in_bytes != std::mem::size_of::<f32>() {
        return Err(ComputeError::ShapeMismatch("Clamp kernel currently only supports f32 data for all inputs"));
    }
    ...
    if !(value_view.data.len() == min_view.data.len() && min_view.data.len() == max_view.data.len()) {
        return Err(ComputeError::ShapeMismatch("Input buffers for Clamp must have the same byte length"));
    }
    if !(value_view.shape == min_view.shape && min_view.shape == max_view.shape) {
        return Err(ComputeError::ShapeMismatch("Input buffers for Clamp must have the same shape"));
    }
    ...
    let output_values: Vec<f32> = value_values.iter()
        .zip(min_values.iter())
        .zip(max_values.iter())
...
mod tests {
    ...
    fn mock_clamp_clamps_values() {
        ...
        let expected_output_data: Vec<f32> = value_data.iter()
        ...
        let result_buffers = cpu.dispatch(&Kernel::Clamp, &[value_buffer_view, min_values_buffer_view, max_values_buffer_view, output_buffer_view, config_buffer_view], workgroups)
